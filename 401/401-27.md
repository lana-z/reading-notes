...
- IF using default user, handle the red `python manage.py migrate`
  - migrations are the rules that apply to database interactions
  - IF using custom user, wait until after models are created.
...
class CustomUserCreationForm(UserCreationForm):
    class Meta # this one must be named Meta
      model = CustomUser
...
from django.contrib.auth.admin imoprt UserAdmin

from .forms import CustomUserCreationForm, CustomUserChangeForm
from .models import CustomUser

class CustomUserAdmin(UserAdmin):
  add_form = CustomUserCreationForm #tells not to use default, use this one
...
in App/ settings.py
```python
login redirect url CHECK REPO FOR THIS
logout redirect url
```
...
- in Project/ settings.py add to INSTALLED APPS
- in appropriate Templates
  - CHECK DEMO
